...
- **Security Headers**: CSP, HSTS, X-Frame-Options, X-Content-Type-Options
  ...
  - IMPORTANT (adapter-static architecture): the production CSP is generated by
    SvelteKit from `kit.csp` in `svelte.config.js` (hash mode — it must carry the
    hash of SvelteKit's inline hydration script; a hosting-header CSP cannot).
    `firebase.json` intentionally serves only `frame-ancestors 'none'` as its CSP
    plus the non-CSP headers. `src/hooks.server.ts` only runs in dev/preview —
    it is NOT active on the deployed static site. Do not re-add a full CSP to
    `firebase.json`: it would combine with (intersect) the meta CSP and block
    hydration.
...
**Common Issues**:
- **CSP Violations**: Add domain to `kit.csp` in `svelte.config.js` (production) and `security.ts` (dev hooks)
...
|---------|-------------------|-------------------|---------------|
| Security Headers | `firebase.json` (prod) / `src/hooks.server.ts` (dev) | `src/lib/config/security.ts` | `SECURITY_HARDENING.md` |
| CSP | `svelte.config.js` `kit.csp` (prod) / `src/lib/config/security.ts` (dev) | — | `SECURITY_SETUP.md` → Customization |

...
The boilerplate includes security rules that are already wired into `firebase.json` (`"firestore"` and `"storage"` blocks), so deploying them just works:
...
**Note**: `npm run deploy` only deploys hosting — deploy rules explicitly (as above) whenever they change, or run a plain `firebase deploy` to ship everything.
...
### ✅ Security Headers
- **Location**: `firebase.json` (headers), `svelte.config.js` `kit.csp` (production CSP), `src/hooks.server.ts` (dev-only)
- **What**: Hash-mode CSP, HSTS, X-Frame-Options, etc.
- **Action Needed**: None - already configured
- **Customization**: Add your domains to `kit.csp` in `svelte.config.js` (production) and mirror in `src/lib/config/security.ts` (dev)

### ✅ Firebase Security Rules Templates
- **Location**: `storage.rules`, `firestore.rules` — wired into `firebase.json` so they actually deploy
- **What**: User segregation, file validation, RBAC (`roles` array + boolean flag claims, matching `app.config.ts` `claimMap`)
...
The **production** CSP is generated by SvelteKit from `kit.csp` in `svelte.config.js` (hash mode, delivered as a `<meta>` tag). Edit it there:
...
kit: {
  csp: {
    mode: 'hash',
    directives: {
      'connect-src': ['self', 'https://*.googleapis.com', 'https://your-api-domain.com'],
      // ... rest
    }
  }
}
...
Then mirror the change in `src/lib/config/security.ts`, which drives the **dev-server** CSP via `src/hooks.server.ts` (not active on the deployed static site). Do not add a full CSP to `firebase.json` — it would intersect with the meta CSP and block hydration.
...
export const APP_CONFIG = {
  auth: {
    roles: { USER: 'user', CLIENT: 'client', ADMIN: 'admin', SYS_ADMIN: 'sysadmin' },
    roleHierarchy: {
      sysadmin: ['admin', 'client', 'user'],
      admin: ['client', 'user'],
      client: ['user'],
    },
    // Maps role value → boolean JWT claim key
    claimMap: { client: 'isClient', admin: 'isAdmin', sysadmin: 'isSysAdmin' },
    // ... permissions
  }
}
```

**Claim scheme** (must stay in lockstep across `app.config.ts`, `firestore.rules`, `storage.rules`, and the bpsk `manage-auth-users` CLI): tokens carry a `roles` array of role names (`['client', 'admin']`) plus boolean flags (`isAdmin`, …). The rules accept either form:

```javascript
// firestore.rules — mirrors rbacUtil.hasRole()
function hasRole(role) {
  return isAuthenticated() &&
         (role in request.auth.token.get('roles', []) ||
          request.auth.token.get(roleFlag(role), false) == true);
}
```

If you add or rename roles, update the `roleFlag()` map in `firestore.rules` and the `isAdmin()` helper in `storage.rules` to match your `claimMap`.
...
If using RBAC, set custom claims with the bpsk CLI (uses a locally-held service account — never wire it into a server endpoint):

```bash
npx @xbg.solutions/bpsk   # → manage-auth-users → Add common roles
```

Or via Cloud Functions, using the same scheme (roles array + boolean flags):

```javascript
// Example: Set default claims on account creation
exports.setUserRole = functions.auth.user().onCreate(async (user) => {
  await admin.auth().setCustomUserClaims(user.uid, {
    roles: ['user'],
    isClient: false, isConsultant: false, isAdmin: false, isSysAdmin: false,
  });
});
```
...
- Verify user has required custom claims (if using RBAC) — a `roles` array of role names and/or boolean flags (`isAdmin`, …); set them via the bpsk `manage-auth-users` CLI

### CSP blocking resources
- Check browser console for violation messages
- Add allowed domains to `kit.csp` in `svelte.config.js` (production CSP)
- Mirror the change in `src/lib/config/security.ts` (dev-server CSP)
- Do NOT add a full CSP to `firebase.json` — it would intersect with the meta CSP and block hydration

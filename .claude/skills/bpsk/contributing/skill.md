...
2. **CLI registry** — `packages/create-frontend/src/registry/` + `manifest.json` (what `npx xbg-frontend add` copies from)
...
Extended atoms are complex UI primitives with significant logic (e.g., OtpInput, Calendar, DataTable). They live in the CLI registry and are installed via `npx xbg-frontend add <name>`.
...
- Import `cn` from `$lib/utils/cn` — **never** from `@xbg.solutions/frontend-core`
...
**Purpose:** Tells the `npx xbg-frontend add` command what to copy and where.
...
The `@xbg.solutions/frontend-core` import is for runtime code (services, stores, types, utils) that consumers import from `node_modules`. Components are copy-on-install — they must use project-local `$lib/` paths.
...
| Mistake | Fix |
|---|---|
| Using `@xbg.solutions/frontend-core` for `cn` in a component | Use `$lib/utils/cn` — components are copied to consuming projects |

...
// ─── app.config.ts template (used when the file does not yet exist) ─────────
function generateAppConfigTemplate(rolesBlock, featuresBlock) {
  // Indent the SETUP block content to match the surrounding structure (4 spaces)
  const indentedRoles = rolesBlock.split('\n').map(l => l ? '    ' + l : l).join('\n');
  const indentedFeatures = featuresBlock.split('\n').map(l => l ? '    ' + l : l).join('\n');
...
async function stepGenerate(cfg) {
  ...
  // Build the roles and features blocks (shared by update and create paths)
  ...
  const featuresBlock = generateFeaturesBlock(featuresObj);
  ...
  if (fs.existsSync(APP_CFG)) {
    ...
    const content = generateAppConfigTemplate(rolesBlock, featuresBlock);
    fs.writeFileSync(APP_CFG, content);
    log.ok('app.config.ts created (from template)');
  }

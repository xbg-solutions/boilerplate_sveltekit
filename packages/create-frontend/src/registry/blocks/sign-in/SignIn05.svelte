<!--
  SignIn05.svelte
  Dark testimonial panel left (h-screen) + centered form right (no card).
  Desktop: left ~35%, right 65%. Mobile: form only.
-->
<script lang="ts">
  ...
  let {
    ...
    onapple = () => {},
    testimonial = {
      quote: 'This platform has completely changed how we work together. It\'s a game-changer.',
      author: 'Marcus Chen',
...
<div class={cn('flex min-h-screen bg-background', className)}>
  ...
  <div class="hidden h-screen flex-col justify-between bg-black text-white px-8 py-12 lg:flex lg:w-[35%]">
    <!-- Logo -->
    <div class="flex h-8 w-8 items-center justify-center rounded bg-white text-black">
      <svg
  ...
  <!-- Form panel -->
  <div class="flex w-full flex-col justify-center bg-gray-50 px-6 py-12 lg:w-[65%] lg:px-12">
    <div class="mx-auto w-full max-w-sm">
      <!-- Logo (mobile) -->
      <div class="mb-8 flex lg:hidden h-8 w-8 items-center justify-center rounded bg-foreground text-background">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="16"
          height="16"
      ...
      <form onsubmit={handleSubmit} class="flex flex-col gap-4 mb-6">
        <div class="flex flex-col gap-1.5">
          <label for="email" class="text-sm font-medium">Email or surname</label>
          <input
            id="email"
            type="email"
            ...
            bind:value={email}
            required
            class="rounded-md border border-input bg-white px-3 py-2 text-sm outline-none placeholder:text-muted-foreground focus:ring-2 focus:ring-ring focus:ring-offset-0"
          />
        </div>
      ...
      <div class="flex gap-2 mb-6">
        <button
          type="button"
          onclick={ogithub}
          class="flex-1 flex items-center justify-center rounded-md border border-input bg-white px-3 py-2 text-sm font-medium transition-colors hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-0"

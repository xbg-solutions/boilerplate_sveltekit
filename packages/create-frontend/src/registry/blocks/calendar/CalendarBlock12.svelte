...
<div class={cn('rounded-xl border bg-background p-6 shadow-sm', className)}>
  ...
  <div class="mb-4 flex items-center justify-between">
    <button type="button" onclick={prev} class="flex h-8 w-8 items-center justify-center rounded-full border hover:bg-muted">
      <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m15 18-6-6 6-6"/></svg>
    </button>
    <span class="text-sm font-medium">{MONTHS[viewMonth]} {viewYear} – {MONTHS[m2]} {y2}</span>
    <button type="button" onclick={next} class="flex h-8 w-8 items-center justify-center rounded-full border hover:bg-muted">

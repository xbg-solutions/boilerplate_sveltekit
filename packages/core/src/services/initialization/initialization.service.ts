 ...
 *
 * External services (authService, tabSyncService) are injected
 * via the options parameter to avoid circular dependencies.
 */
...
import { publish, subscribe } from '../events/pub-sub';
...
/**
 * Interface for an injectable auth service
 */
export interface IAuthServiceDep {
  initialize(): Promise<void>;
}

/**
 * Interface for an injectable tab sync service
 */
export interface ITabSyncServiceDep {
  initialize(): Promise<void>;
}
...
export interface InitializationOptions {
  ...
  /** Auth service instance (injected to avoid circular deps) */
  authService?: IAuthServiceDep;
  /** Tab sync service instance (injected to avoid circular deps) */
  tabSyncService?: ITabSyncServiceDep;
}
...
export function createInitializationService() {
   ...
   */
  const initialize = async (options: InitializationOptions): Promise<void> => {
    // Default options
    const {
      firebaseConfig,
      useEmulators = false,
    ...
    try {
      ...
      // Initialize Firebase Auth (if auth service provided)
      await initializeAuth(useEmulators, authService);

      // Initialize Tab Sync service for cross-tab communication (if provided)
      if (tabSyncService) {
        try {

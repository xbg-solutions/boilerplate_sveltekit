<!--
  CalendarBlock11.svelte
  Dual-month range picker with custom availability/restriction message below.
-->
<script lang="ts">
  ...
  let {
    ...
    startDate = null,
    endDate = null,
    availabilityMessage = 'We are open in June and July only.'
  }: {
    class?: string;
    startDate?: Date | null;
    endDate?: Date | null;
...
<div class={cn('inline-flex flex-col rounded-md border p-4', className)}>
  ...
  {#if availabilityMessage}
    <p class="mt-3 text-center text-xs text-muted-foreground">{availabilityMessage}</p>
  {/if}
</div>
